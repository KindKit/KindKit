...
public struct FloatStringFormatter< FloatType : BinaryFloatingPoint > : IStringFormatter {
    ...
    public func format(_ input: FloatType) -> String {
        if let value = Float(exactly: input) {
            if let string = self.formatter.string(from: NSNumber(value: value)) {
                return string
            }
        } else if let value = Double(exactly: input) {
            if let string = self.formatter.string(from: NSNumber(value: value)) {
                return string
            }
        }
