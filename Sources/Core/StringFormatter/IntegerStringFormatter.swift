...
public struct IntegerStringFormatter< IntegerType : BinaryInteger > : IStringFormatter {
    ...
    public func format(_ input: IntegerType) -> String {
        if let value = Int(exactly: input) {
            if let string = self.formatter.string(from: NSNumber(value: value)) {
                return string
            }
        } else if let value = UInt(exactly: input) {
            if let string = self.formatter.string(from: NSNumber(value: value)) {
                return string
            }
        }
