...
public final class Condition< ThenType : IEntity & IStorable, ElseType : IEntity & IStorable > : IEntity where ThenType.StoreType == ElseType.StoreType {
    ...
    public let onStore = Signal< Void, [ThenType.StoreType] >()
    ...
    public init(
        ...
        self._then = then
        self._else = `else`
        self._then.onVariants.add(self, {
            $0.variants = $1
        })
        self._then.onStore.add(self, {
            $0.store = $1
        })
        self._else.onVariants.add(self, {
            $0.variants = $1
        })
        self._else.onStore.add(self, {
            $0.store = $1
        })
    }
