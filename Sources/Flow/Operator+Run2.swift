        ...
        Pipeline1.Output.Failure == Pipeline2.Output.Failure
    {
        ...
        init(
            ...
            self._subscription1 = pipeline1.subscribe(
                onReceiveValue: { [unowned self] in self._receive1(value: $0) },
                onReceiveError: { [unowned self] in self._receive1(error: $0) },
                onCompleted: { [unowned self] in self._completed() }
            )
            self._subscription2 = pipeline2.subscribe(
                onReceiveValue: { [unowned self] in self._receive2(value: $0) },
                onReceiveError: { [unowned self] in self._receive2(error: $0) },
                onCompleted: { [unowned self] in self._completed() }
            )
        }
...
private extension Operator.Run2 {
...
extension IFlowOperator {
    ...
    >(
        pipeline1: Pipeline1,
        pipeline2: Pipeline2
    ) -> Operator.Run2< Pipeline1, Pipeline2 > where
        Pipeline1.Input == Pipeline2.Input,
        Pipeline1.Output.Failure == Pipeline2.Output.Failure
    {
        let next = Operator.Run2< Pipeline1, Pipeline2 >(pipeline1, pipeline2)
...
public extension Flow {
    ...
    >(
        pipeline1: Pipeline1,
        pipeline2: Pipeline2
    ) -> Builder.Head< Operator.Run2< Pipeline1, Pipeline2 > > where
...
public extension Builder.Head {
    ...
    >(
        pipeline1: Pipeline1,
        pipeline2: Pipeline2
    ) -> Builder.Chain< Head, Operator.Run2< Pipeline1, Pipeline2 > > where
