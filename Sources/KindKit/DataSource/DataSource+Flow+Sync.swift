    ...
    > : IFlowOperator {
        ...
        public func receive(value: Input.Success) {
            if self._dataSource.isNeedSync == true {
                ...
                self._finish(value, self._dataSource.result)
            }
        }
...
extension IFlowOperator {
    ...
    ) -> KindKit.DataSource.Flow.Sync< Output, Success, Failure, DataSource > where
        Output.Failure == Failure
    {
        let next = KindKit.DataSource.Flow.Sync< Output, Success, Failure, DataSource >(dataSource, success, failure)
    ...
    >(
        _ dataSource: DataSource,
        _ success: @escaping (Output.Success, DataSource.Success) -> Success
    ) -> KindKit.DataSource.Flow.Sync< Output, Success, DataSource.Failure, DataSource > where
        Output.Failure == DataSource.Failure
    {
        let next = KindKit.DataSource.Flow.Sync< Output, Success, DataSource.Failure, DataSource >(dataSource, success)
        self.subscribe(next: next)
        return next
    }
    ...
    >(
        _ dataSource: DataSource,
        _ failure: @escaping (Output.Success, DataSource.Failure) -> Failure
    ) -> KindKit.DataSource.Flow.Sync< Output, DataSource.Success, Failure, DataSource > where
        Output.Failure == Failure
    {
        let next = KindKit.DataSource.Flow.Sync< Output, DataSource.Success, Failure, DataSource >(dataSource, failure)
        self.subscribe(next: next)
        return next
    }
    ...
    >(
        _ dataSource: DataSource
    ) -> KindKit.DataSource.Flow.Sync< Output, DataSource.Success, DataSource.Failure, DataSource > where
        Output.Failure == DataSource.Failure
    {
        let next = KindKit.DataSource.Flow.Sync< Output, DataSource.Success, DataSource.Failure, DataSource >(dataSource)
        self.subscribe(next: next)
        return next
    }
...
public extension Flow.Builder {
    ...
    ) -> Flow.Head.Builder< KindKit.DataSource.Flow.Sync< Input, Success, Input.Failure, DataSource > > {
    ...
    >(
        dataSource: DataSource,
        success: @escaping (Input.Success, DataSource.Success) -> Success
    ) -> Flow.Head.Builder< KindKit.DataSource.Flow.Sync< Input, Success, DataSource.Failure, DataSource > > where
    ...
    >(
        dataSource: DataSource,
        failure: @escaping (Input.Success, DataSource.Failure) -> Input.Failure
    ) -> Flow.Head.Builder< KindKit.DataSource.Flow.Sync< Input, DataSource.Success, Input.Failure, DataSource > > {
    ...
    >(
        dataSource: DataSource
    ) -> Flow.Head.Builder< KindKit.DataSource.Flow.Sync< Input, DataSource.Success, DataSource.Failure, DataSource > > where
        Input.Failure == DataSource.Failure
    {
...
public extension Flow.Head.Builder {
    ...
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Head.Output, Success, Failure, DataSource > > where
    ...
    >(
        dataSource: DataSource,
        success: @escaping (Head.Output.Success, DataSource.Success) -> Success
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Head.Output, Success, DataSource.Failure, DataSource > > where
    ...
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Head.Output, DataSource.Success, Head.Output.Failure, DataSource > > {
    ...
    >(
        dataSource: DataSource
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Head.Output, DataSource.Success, DataSource.Failure, DataSource > > where
        Head.Output.Failure == DataSource.Failure
    {
...
public extension Flow.Chain.Builder {
    ...
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Tail.Output, Success, Failure, DataSource > > where
    ...
    >(
        dataSource: DataSource,
        success: @escaping (Tail.Output.Success, DataSource.Success) -> Success
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Tail.Output, Success, DataSource.Failure, DataSource > > where
    ...
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Tail.Output, DataSource.Success, Tail.Output.Failure, DataSource > > {
    ...
    >(
        dataSource: DataSource
    ) -> Flow.Chain.Builder< Head, KindKit.DataSource.Flow.Sync< Tail.Output, DataSource.Success, DataSource.Failure, DataSource > > where
        Tail.Output.Failure == DataSource.Failure
    {
