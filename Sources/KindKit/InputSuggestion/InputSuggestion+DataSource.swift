...
public extension InputSuggestion {
    ...
    final class DataSource< OutputType : Equatable, RequestType, ResponseType : IApiResponse > : IInputSuggestion, IInputSuggestionStorable {
        ...
        public let onStore = Signal.Args< Void, [OutputType] >()
        ...
        private let _dataSource: KindKit.DataSource.Action.Api< RequestType, ResponseType >
        
        public init(
            prepare: @escaping (String) -> RequestType?,
            map: @escaping (ResponseType.Success) -> [OutputType],
            format: @escaping (OutputType) -> String,
            dataSource: KindKit.DataSource.Action.Api< RequestType, ResponseType >
        ) {
