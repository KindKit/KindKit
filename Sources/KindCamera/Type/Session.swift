...
public final class Session {
    ...
    private let _observer = Observer< IObserver >()
...
private extension Session {
    ...
    func _set(deviceOrientation: Orientation?) {
        if self.deviceOrientation != deviceOrientation {
            ...
            self._observer.emit({ $0.changed(self, deviceOrientation: deviceOrientation) })
        }
    }
    ...
    func _set(interfaceOrientation: Orientation?) {
        if self.interfaceOrientation != interfaceOrientation {
            ...
            self._observer.emit({ $0.changed(self, interfaceOrientation: interfaceOrientation) })
        }
    }
    ...
    func _didStart() {
        self.isStarted = true
        self._observer.emit({ $0.started(self) })
    }

    func _didStop() {
        self.isStarted = false
        self._observer.emit({ $0.stopped(self) })
    }
    ...
    func configure(
        ...
        self._configure(
            ...
            willConfigure: {
                ...
                self.session.stopRunning()
                self._observer.emit({ $0.startConfiguration(self) })
            },
            configureVideoDevice: configureVideoDevice,
            configureAudioDevice: configureAudioDevice,
            didStart: {
                self._subscribeSession()
                self._observer.emit({ $0.finishConfiguration(self) })
                completion()
            }
        )
    }
