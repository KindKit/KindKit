...
public final class VirtualKeyboard {
    ...
    private let _observer = Observer< IVirtualKeyboardObserver >()
    ...
    public func add(observer: IVirtualKeyboardObserver, priority: KindEvent.Priority) {
...
extension VirtualKeyboard {
    ...
    private func _willShow(_ notification: Notification) {
        ...
        self._observer.emit({ $0.willShow(self, info: info) })
    }
    ...
    private func _didShow(_ notification: Notification) {
        guard let userInfo = notification.userInfo else { return }
        guard let info = Info(userInfo) else { return }
        self._observer.emit({ $0.didShow(self, info: info) })
    }
    ...
    private func _willHide(_ notification: Notification) {
        guard let userInfo = notification.userInfo else { return }
        guard let info = Info(userInfo) else { return }
        self._observer.emit({ $0.willHide(self, info: info) })
    }
    ...
    private func _didHide(_ notification: Notification) {
        guard let userInfo = notification.userInfo else { return }
        guard let info = Info(userInfo) else { return }
        self._observer.emit({ $0.didHide(self, info: info) })
    }
