...
protocol KKLongTapGestureDelegate : AnyObject {
...
public final class LongTapGesture : IGesture, IGestureTriggerable {
    ...
    @KindMonadicProperty
    public var minimumDuration = SecondsInterval(0.5) {
        didSet {
            guard self.minimumDuration != oldValue else { return }
            if self.isLoaded == true {
                self.reuse.content.kk_update(minimumDuration: self.minimumDuration)
            }
        }
    }
    ...
    public var allowableMovement: Double = 10 {
        didSet {
            ...
            if self.isLoaded == true {
                self.reuse.content.kk_update(allowableMovement: self.allowableMovement)
            }
        }
    }
...
extension LongTapGesture : KKTapGestureDelegate {
