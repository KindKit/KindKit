...
public extension Operator {
    ...
    final class Completion< Input : IResult, Success, Failure : Swift.Error > : IOperator {
...
private extension Operator.Completion {
...
public extension IBuilder {
    ...
    ) -> Chain< Head, Operator.Completion< Tail.Output, Success, Failure > > {
    ...
    ) -> Chain< Head, Operator.Completion< Tail.Output, Success, Failure > > where Tail.Output.Failure == Never {
    ...
    ) -> Chain< Head, Operator.Completion< Tail.Output, Success, Never > > where Tail.Output.Failure == Never {
        return self.append(.init({ input, completion in
            switch input {
    ...
    ) -> Chain< Head, Operator.Completion< Tail.Output, Success, Tail.Output.Failure > > {
        return self.append(.init({ input, completion in
            switch input {
            case .success(let value): function(value, completion)
    ...
    ) -> Chain< Head, Operator.Completion< Tail.Output, Tail.Output.Success, Failure > > {
        return self.append(.init({ input, completion in
            switch input {
