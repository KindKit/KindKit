...
public extension Text.Strikethrough {
    ...
    init(_ nsUnderlineStyle: NSUnderlineStyle) {
        ...
        if nsUnderlineStyle.contains(.single) == true {
            rawValue |= Text.Strikethrough.single.rawValue
        }
        if nsUnderlineStyle.contains(.thick) == true {
            rawValue |= Text.Strikethrough.thick.rawValue
        }
        if nsUnderlineStyle.contains(.double) == true {
            rawValue |= Text.Strikethrough.double.rawValue
        }
        if nsUnderlineStyle.contains(.patternDot) == true {
            rawValue |= Text.Strikethrough.patternDot.rawValue
        }
        if nsUnderlineStyle.contains(.patternDash) == true {
            rawValue |= Text.Strikethrough.patternDash.rawValue
        }
        if nsUnderlineStyle.contains(.patternDashDot) == true {
            rawValue |= Text.Strikethrough.patternDashDot.rawValue
        }
        if nsUnderlineStyle.contains(.patternDashDotDot) == true {
            rawValue |= Text.Strikethrough.patternDashDotDot.rawValue
        }
        if nsUnderlineStyle.contains(.byWord) == true {
            rawValue |= Text.Strikethrough.byWord.rawValue
        }
