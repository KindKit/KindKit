...
public extension TextStrikethrough {
    ...
    init(_ nsUnderlineStyle: NSUnderlineStyle) {
        ...
        if nsUnderlineStyle.contains(.single) == true {
            rawValue |= TextStrikethrough.single.rawValue
        }
        if nsUnderlineStyle.contains(.thick) == true {
            rawValue |= TextStrikethrough.thick.rawValue
        }
        if nsUnderlineStyle.contains(.double) == true {
            rawValue |= TextStrikethrough.double.rawValue
        }
        if nsUnderlineStyle.contains(.patternDot) == true {
            rawValue |= TextStrikethrough.patternDot.rawValue
        }
        if nsUnderlineStyle.contains(.patternDash) == true {
            rawValue |= TextStrikethrough.patternDash.rawValue
        }
        if nsUnderlineStyle.contains(.patternDashDot) == true {
            rawValue |= TextStrikethrough.patternDashDot.rawValue
        }
        if nsUnderlineStyle.contains(.patternDashDotDot) == true {
            rawValue |= TextStrikethrough.patternDashDotDot.rawValue
        }
        if nsUnderlineStyle.contains(.byWord) == true {
            rawValue |= TextStrikethrough.byWord.rawValue
        }
