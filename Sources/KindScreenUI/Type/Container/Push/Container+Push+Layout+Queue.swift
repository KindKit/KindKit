...
extension Container.Push.Layout {
    ...
    final class Queue : ILayout {
        
        weak var delegate: ILayoutDelegate?
        weak var appearedView: IView?
        var state: Container.Push.State = .empty {
            didSet {
...
private extension Container.Push.Layout.Queue {
    
    func _beginRect(bounds: Rect, item: Container.PushItem) -> Rect {
    ...
    func _endRect(bounds: Rect, item: Container.PushItem) -> Rect {
    ...
    func _size(item: Container.PushItem) -> Double {
        let inset = self._inset(item: item)
        switch item.container.pushPlacement {
...
extension Container.Push.Layout.Queue {
