...
public extension Container {
    ...
    final class Page< Screen : IPageScreen > : IPageContainer, IContainerScreenable {
        ...
        public private(set) var bar: PageBarView {
            set {
        ...
        public var backward: IPageContentContainer? {
        ...
        public var current: IPageContentContainer? {
            return self._current?.container
        }
        public var forward: IPageContentContainer? {
            guard let current = self._current else { return nil }
            guard let index = self._items.firstIndex(where: { $0 === current }) else { return nil }
        ...
        private var _interactiveBackward: Container.PageItem?
        private var _interactiveCurrent: Container.PageItem?
        private var _interactiveForward: Container.PageItem?
#endif
        private var _items: [Container.PageItem]
        private var _current: Container.PageItem?
...
#endif
            self._items = containers.map({ Container.PageItem($0) })
        ...
        public func parentInset(for container: IContainer) -> Container.AccumulateInset {
            ...
            if self._items.contains(where: { $0.container === container }) == true {
                if self.barHidden == false && Container.BarController.shared.hidden(.page) == false {
        ...
        public func set(containers: [IPageContentContainer], current: IPageContentContainer?, animated: Bool, completion: (() -> Void)?) {
            ...
            self._bar.items(self._items.map({ $0.bar }))
            let newCurrent: Container.PageItem?
            if current != nil {
...
extension Container.Page : IPageBarViewDelegate {
    
    public func pressed(pageBar: PageBarView, item: PageBarView.Item) {
...
extension Container.Page : IRootContentContainer {
}

extension Container.Page : IStackContentContainer where Screen : IScreenStackable {
...
extension Container.Page : IGroupContentContainer where Screen : IScreenGroupable  {
...
extension Container.Page : IDialogContentContainer where Screen : IScreenDialogable {
...
extension Container.Page : IContainerBarControllerObserver {
...
private extension Container.Page {
