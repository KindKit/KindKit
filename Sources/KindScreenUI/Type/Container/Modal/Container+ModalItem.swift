...
extension Container {
    ...
    final class ModalItem {
        ...
        let container: IModalContentContainer
        let owner: AnyObject?
        let view: MaskView
        var currentDetent: DynamicSize.Dimension
        let sheet: KindScreenUI.Modal.Presentation.Sheet!
        
        private var _cacheAvailable: Size?
        private var _cacheSheetDetents: [DynamicSize.Dimension] = []
        private var _cacheSheetSizes: [Size] = []
        
        init(
            _ container: IModalContentContainer,
...
private extension Container.ModalItem {
...
extension Container.ModalItem {
    ...
    func sheetDetentIndex(of detent: DynamicSize.Dimension) -> Int? {
    ...
    func sheetSize(of detent: DynamicSize.Dimension) -> Size? {
...
extension Container.ModalItem {
    ...
    func size(of detent: DynamicSize.Dimension, available: Size) -> Size {
        self._updateCacheIfNeeded(available: available)
        if self.isSheet == true {
...
extension Container.ModalItem : Hashable {
...
extension Container.ModalItem : Equatable {
    
    static func == (lhs: Container.ModalItem, rhs: Container.ModalItem) -> Bool {
