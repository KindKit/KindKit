...
public extension Container {
    ...
    final class Stack< Screen : IStackScreen > : IStackContainer, IContainerScreenable {
        ...
        public var orientation: UIInterfaceOrientation = .unknown {
            didSet {
                ...
                self.root.didChange(orientation: self.orientation)
        ...
        private var _root: Container.StackItem
        private var _items: [Container.StackItem] = []
        private var _layout: Layout
        private var _view: CustomView
#if os(iOS)
        private var _interactiveGesture = EdgePanGesture(.left)
        ...
        private var _interactiveBackward: Container.StackItem?
        private var _interactiveCurrent: Container.StackItem?
#endif
        @inline(__always)
        private var _current: Container.StackItem {
...
#endif
            self._root = Container.StackItem(root)
        ...
        public func parentInset(for container: IContainer) -> Container.AccumulateInset {
            ...
            if let item = item {
                if item.barHidden == false && Container.BarController.shared.hidden(.stack) == false {
        ...
        public func contentInset() -> Container.AccumulateInset {
            func _inset_(item: Container.StackItem) -> Container.AccumulateInset {
        ...
        public func close(container: IContainer, animated: Bool, completion: (() -> Void)?) -> Bool {
            if let container = container as? IStackContentContainer {
        ...
        public func set(root: IStackContentContainer, animated: Bool, completion: (() -> Void)?) {
            let parentInset = self.parentInset()
            let newRootItem = Container.StackItem(root, inset: parentInset.interactive)
        ...
        public func set(containers: [IStackContentContainer], animated: Bool, completion: (() -> Void)?) {
            if containers.isEmpty == false {
                ...
                self._items = containers.map({ container in
                    ...
                    return Container.StackItem(container, inset: parentInset.interactive)
                })
                for item in self._items {
        ...
        public func push(container: IStackContentContainer, animated: Bool, completion: (() -> Void)?) {
            ...
            let parentInset = self.parentInset()
            let forward = Container.StackItem(container, inset: parentInset.interactive)
            let current = self._current
        ...
        public func push(containers: [IStackContentContainer], animated: Bool, completion: (() -> Void)?) {
            ...
            if newContainers.count > 0 {
                let current = self._current
                let parentInset = self.parentInset()
                let items: [Container.StackItem] = newContainers.map({ Container.StackItem($0, inset: parentInset.interactive) })
                for item in items {
                    item.container.parent = self
                }
        ...
        public func push< Wireframe : IWireframe >(wireframe: Wireframe, animated: Bool, completion: (() -> Void)?) where Wireframe : AnyObject, Wireframe.Container : IStackContentContainer {
            ...
            let parentInset = self.parentInset()
            let forward = Container.StackItem(wireframe.container, wireframe, inset: parentInset.interactive)
            let current = self._current
            self._items.append(forward)
...
extension Container.Stack : IRootContentContainer {
}

extension Container.Stack : IGroupContentContainer where Screen : IScreenGroupable {
...
extension Container.Stack : IDialogContentContainer where Screen : IScreenDialogable {
...
extension Container.Stack : IModalContentContainer where Screen : IScreenModalable {
...
extension Container.Stack : IContainerBarControllerObserver {
...
private extension Container.Stack {
    ...
    func _push(
        current: Container.StackItem,
        forward: Container.StackItem,
        animated: Bool,
        completion: (() -> Void)?
    ) {
        let hideBar: Bool
        let groupBarOldVisibility = Container.BarController.shared.visibility(.group)
            ...
            self._animation = KindAnimation.default.run(
                .custom(
                    ...
                    processing: { [weak self] progress in
                        ...
                        if hideBar == true {
                            Container.BarController.shared.set(.group, visibility: groupBarOldVisibility.lerp(groupBarNewVisibility, progress: progress))
                        } else {
                    ...
                    completion: { [weak self] in
                        ...
                        if hideBar == true {
                            Container.BarController.shared.set(.group, visibility: groupBarNewVisibility)
                        } else {
                            self.refreshContentInset()
                        }
#if os(iOS)
...
private extension Container.Stack {
    ...
    func _beginInteractiveGesture() {
        ...
        if backward === self._root && self.hidesGroupBarWhenPushed == true {
            ...
            self._interactiveGroupBarOldVisibility = Container.BarController.shared.visibility(.group)
    ...
    func _changeInteractiveGesture() {
        ...
        if self._interactiveGroupBottomBar == true {
            Container.BarController.shared.set(.group, visibility: self._interactiveGroupBarOldVisibility.lerp(self._interactiveGroupBarNewVisibility, progress: progress))
        } else {
            self.refreshContentInset()
        }
    }
            ...
            self._animation = KindAnimation.default.run(
                .custom(
                    ...
                    processing: { [weak self] progress in
                        ...
                        self._layout.updateIfNeeded()
                        if self._interactiveGroupBottomBar == true {
                            Container.BarController.shared.set(.group, visibility: self._interactiveGroupBarOldVisibility.lerp(self._interactiveGroupBarNewVisibility, progress: progress.invert))
                        } else {
                            self.refreshContentInset()
                        }
                    },
    ...
    func _finishInteractiveAnimation(backward: Container.StackItem, current: Container.StackItem) {
        ...
        current.container.parent = nil
        if self._interactiveGroupBottomBar == true {
            Container.BarController.shared.set(.group, visibility: self._interactiveGroupBarNewVisibility)
        } else {
            self.refreshContentInset()
        }
#if os(iOS)
    ...
    func _cancelInteractiveAnimation(backward: Container.StackItem, current: Container.StackItem) {
        self._resetInteractiveAnimation()
        ...
        if self._interactiveGroupBottomBar == true {
            Container.BarController.shared.set(.group, visibility: self._interactiveGroupBarOldVisibility)
        } else {
            self.refreshContentInset()
        }
#if os(iOS)
