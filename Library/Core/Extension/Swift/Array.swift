...
public extension Array {
    ...
    @inlinable
    func kk_uniqued() -> Self where Element : AnyObject {
        return self.kk_uniqued(where: { $0 === $1 })
    }
    
    @inlinable
    func kk_uniqued() -> Self where Element : Comparable {
        return self.kk_uniqued(where: { $0 == $1 })
    }
    ...
    func kk_reduce< Result >(
        _ empty: () -> Result,
        _ first: (Element) -> Result,
        _ next: (Result, Element) -> Result
    ) -> Result {
        if self.count > 1 {
            return self[1 ..< self.endIndex].reduce(first(self[0]), next)
    ...
    @inlinable
    func kk_reduce< Result >(
        _ empty: () throws -> Result,
        _ first: (Element) throws -> Result,
        _ next: (Result, Element) throws -> Result
    ) rethrows -> Result {
        if self.count > 1 {
            return try self[1 ..< self.endIndex].reduce(try first(self[0]), next)
        } else if self.count > 0 {
            return try first(self[0])
        }
        return try empty()
    }
