...
public extension Bundle {
    ...
    func kk_containsUrlSheme(url: URL) -> Bool {
        ...
        guard let urlTypes = self.infoDictionary?["CFBundleURLTypes"] as? [Any] else { return false }
        urlTypes.forEach({
    ...
    func kk_bundle(language: String) -> Bundle? {
        guard let path = self.path(forResource: language, ofType: "lproj") else { return nil }
