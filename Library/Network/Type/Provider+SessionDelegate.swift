...
extension Provider {
    ...
    final class SessionDelegate : NSObject {
        ...
        private var _tasks: [Int : TaskQuery] = [:]
        ...
        func append(_ taskQuery: TaskQuery) {
            self._lock.perform({
...
extension Provider.SessionDelegate : @unchecked Sendable {
}

private extension Provider.SessionDelegate {
    
    func _query(task: URLSessionTask) -> TaskQuery? {
        return self._lock.perform({
            return self._tasks[task.taskIdentifier]
        })
    }
    
    func _remove(task: URLSessionTask) -> TaskQuery? {
        return self._lock.perform({
            return self._tasks.removeValue(forKey: task.taskIdentifier)
        })
    }
    
    func _move(fromTask: URLSessionTask, toTask: URLSessionTask) -> TaskQuery? {
        return self._lock.perform({
