...
public extension Container {
    ...
    final class Modal : IModalContainer {
        ...
        public var previous: IModalContentContainer? {
            return self._previous?.container
        }
        public var current: IModalContentContainer? {
            return self._current?.container
        }
        public var animationVelocity: Double
        
        private var _hookView: CustomView
        private var _layout: Layout
        private var _view: CustomView
        private var _tapGesture = TapGesture().enabled(false)
#if os(iOS)
        private var _interactiveGesture = PanGesture().enabled(false)
        private var _interactiveBeginLocation: Point?
#endif
        private var _items: [Container.ModalItem]
        private var _previous: Container.ModalItem?
        private var _current: Container.ModalItem? {
            didSet {
        ...
        public func contentInset() -> Container.AccumulateInset {
            ...
            case .transition(let modal, let from, let to, let progress):
                ...
                let finalInset: Container.AccumulateInset
                switch (from, to) {
                case (.some, .none):
        ...
        public func close(container: IContainer, animated: Bool, completion: (() -> Void)?) -> Bool {
            if let container = container as? IModalContentContainer {
        ...
        public func present(container: IModalContentContainer, animated: Bool, completion: (() -> Void)?) {
            ...
            let item = Container.ModalItem(container)
        ...
        public func present< Wireframe : IWireframe >(wireframe: Wireframe, animated: Bool, completion: (() -> Void)?) where Wireframe : AnyObject, Wireframe.Container : IModalContentContainer {
            wireframe.container.parent = self
            let item = Container.ModalItem(wireframe.container, wireframe)
            self._items.append(item)
            if self._current == nil && self._animation == nil {
                self._present(modal: item, animated: animated, completion: completion)
            } else {
        ...
        @discardableResult
        public func dismiss(container: IModalContentContainer, animated: Bool, completion: (() -> Void)?) -> Bool {
        ...
        @discardableResult
        public func dismiss< Wireframe : IWireframe >(wireframe: Wireframe, animated: Bool, completion: (() -> Void)?) -> Bool where Wireframe : AnyObject, Wireframe.Container : IModalContentContainer {
...
extension Container.Modal : IRootContentContainer {
}

private extension Container.Modal {
    ...
    func _present(current: Container.ModalItem?, next: Container.ModalItem, animated: Bool, completion: (() -> Void)?) {
    ...
    func _present(modal: Container.ModalItem, animated: Bool, completion: (() -> Void)?) {
        self._current = modal
    ...
    func _dismiss(current: Container.ModalItem, previous: Container.ModalItem?, animated: Bool, completion: (() -> Void)?) {
    ...
    func _dismiss(modal: Container.ModalItem, animated: Bool, completion: (() -> Void)?) {
        if self.isPresented == true {
