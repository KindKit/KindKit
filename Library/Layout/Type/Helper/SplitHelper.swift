...
extension SplitHelper {
    ...
    static func hSize(
        ...
        if invalidIndecies.count > 0 {
            maximum = Coordinate.zero
            accumulate = Coordinate.zero
...
fileprivate extension SplitHelper {
    ...
    @inline(__always)
    static func _hSizePurpose(of base: SizeRequest, spacing: Coordinate, count: Int) -> SizeRequest {
        let space = Self._space(
            of: base.available.width.validated(default: base.container.width),
            spacing: spacing,
            count: count
        )
        return .init(
    ...
    @inline(__always)
    static func _vSizePurpose(of base: SizeRequest, spacing: Coordinate, count: Int) -> SizeRequest {
        let space = Self._space(
            of: base.available.height.validated(default: base.container.height),
            spacing: spacing,
            count: count
        )
        return .init(
    ...
    @inline(__always)
    static func _space(of available: Coordinate, spacing: Coordinate, count: Int) -> Coordinate {
        if count > 1 {
            return (available - (spacing * Coordinate(count - 1))) / Coordinate(count)
        } else if count > 0 {
            return available / Coordinate(count)
        }
        return .zero
    }
