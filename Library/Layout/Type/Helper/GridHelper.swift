    ...
    static func vFrames(
        ...
        return .init(
            final: .init(
                origin: purpose.container.origin,
                size: .init(
                    width: purpose.available.width.validated(default: accumulateSize.width),
...
fileprivate extension GridHelper {
    ...
    static func _widths(
        ...
        let available = purpose.available.width.validated(default: purpose.container.width)
        ...
        guard fitColumns > 0 else {
            let spacing = spacing.x * Coordinate(columns.count - 1)
            let width = (available - spacing) / Coordinate(columns.count)
            return .init(
                repeating: width,
                count: columns.count
            )
        }
        var accumulator: [Coordinate] = .init(
            repeating: 0,
            count: columns.count
        )
        for index in 0 ..< contentCount {
        ...
        let fitSpacing = spacing.x * Coordinate(fitColumns)
        let fitWidth = accumulator.reduce(0, { $0 + $1 })
        let leftWidth = available - (fitWidth + fitSpacing)
        let autoSpacing = spacing.x * Coordinate(autoColumns - 1)
        let autoWidth = (leftWidth - autoSpacing) / Coordinate(autoColumns)
