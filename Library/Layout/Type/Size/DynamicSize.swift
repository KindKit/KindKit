...
public extension DynamicSize {
    ...
    func resolve(
        ...
        case (.fixed(let w), .fixed(let h)):
            return .init(width: w, height: h)
        case (.fixed(let w), .fill):
            let h = request.available.height.validated(default: request.container.height)
            return .init(width: w, height: h)
        case (.fixed(let w), .fit):
            let h = request.available.height
            return calculate(.init(width: w, height: h))
        case (.fill, .fixed(let h)):
            let w = request.available.width.validated(default: request.container.width)
            return .init(width: w, height: h)
        case (.fill, .fill):
            let w = request.available.width.validated(default: request.container.width)
            let h = request.available.height.validated(default: request.container.height)
    ...
    func resolve(
        by request: SizeRequest,
        withWidth: (Coordinate, Coordinate) -> Size,
        withHeight: (Coordinate, Coordinate) -> Size,
        withSize: (Size) -> Size
    ) -> Size {
        switch (self.width, self.height) {
        case (.fixed(let w), .fixed(let h)):
        ...
        case (.fit, .fill):
            ...
            return withHeight(h, request.available.width)
        case (.fit, .fit):
            return withSize(request.available.validated(default: request.container))
        }
    }
    
}

extension DynamicSize : MapTrait {
}
