...
public final class MovieRecorder : Recorder {
    ...
    public var interfaceOrientation: Orientation? {
        didSet {
    ...
    private var _restorePreset: VideoDevice.Preset?
    private var _restoreFrameDuration: VideoDevice.FrameDuration?
    private var _restoreFlashMode: VideoDevice.Torch?
    ...
    public nonisolated func attach(session: Session) {
        guard self.isAttached == false else { return }
        self.session = session
    }
    
    public nonisolated func detach() {
...
extension MovieRecorder : Equatable {
    
    public nonisolated static func == (lhs: MovieRecorder, rhs: MovieRecorder) -> Bool {
        return lhs === rhs
    }
    
}

extension MovieRecorder : @unchecked Sendable {
}
...
public extension MovieRecorder {
    ...
    func start(
        config: Config = .init(),
        onSuccess: @escaping @Sendable (TemporaryFile) -> Void,
...
private extension MovieRecorder {
    ...
    func _restore(_ completion: @escaping @Sendable () -> Void) {
...
extension MovieRecorder {
    ...
    func finish(_ file: TemporaryFile) {
