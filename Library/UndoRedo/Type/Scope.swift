...
final class Scope {
    ...
    func create(_ command: String, _ object: String, _ closure: (inout IMutatingPermanentContext) -> Void) {
        let appended: Bool
    ...
    func update(_ command: String, _ object: String, _ closure: (inout IMutatingTransformContext) -> Void) {
        let appended: Bool
        if let state = self._states.last {
    ...
    func delete(_ command: String, _ object: String, _ closure: (inout IMutatingPermanentContext) -> Void) {
        let appended: Bool
        if let state = self._states.last {
