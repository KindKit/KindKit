...
extension Document.Node : DebugTrait {
    
    public func buildInfo() -> Info {
        return ObjectInfo(name: "Node", sequenceBuilder: {
            KeyValueInfo(key: "Name", value: self.name)
            if self.attributes.count > 0 {
                KeyValueInfo(key: "Attributes", value: self.attributes)
            }
            if self.nodes.count > 0 {
                KeyValueInfo(key: "Nodes", value: self.nodes)
            }
            if let value = self.value {
                KeyValueInfo(key: "Value", value: value)
            }
        })
    }
