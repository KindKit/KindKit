...
@Monadic
public final class ScrollHideableBar< Layout : ILayout > {
    ...
    @MonadicField
    public var source: ScrollView< Layout >? {
        willSet {
    ...
    private var _animation: CancelTrait? {
...
private extension ScrollHideableBar {
    ...
    final func _onScrollToTop() {
        self._show(animate: true, completion: nil)
    }

    final func _onDragging() {
    ...
    final func _onEndDragging(decelerating: Bool) {
        if decelerating == false {
            self._end()
        }
    }
    
    final func _onEndDecelerating() {
        self._end()
    }
...
private extension ScrollHideableBar {
    ...
    @inline(__always)
    final func _show(animate: Bool, completion: (() -> Void)?) {
        switch self.state {
        case .show:
            self.state = .show
            completion?()
    ...
    @inline(__always)
    final func _hide(animate: Bool, completion: (() -> Void)?) {
        switch self.state {
        case .show:
            if animate == true {
                self._animation = KindAnimation.default.run(
