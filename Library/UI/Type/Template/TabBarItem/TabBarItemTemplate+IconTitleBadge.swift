...
extension TabBarItemTemplate {
    ...
    > : ITemplate {
        ...
        public var spacing: Double {
            nonmutating set { self.layout.content.content.center.inset.top = newValue }
            get { self.layout.content.content.center.inset.top }
        }
        ...
        public init(
            ...
            self.layout = MarginLayout(
                AnchorLayout(
                    content: VAccessoryLayout(
                        leading: ViewLayout(icon),
                        center: MarginLayout(
                            ViewLayout(title)
                        ).update(on: {
                            $0.inset.top = spacing ?? Defaults.spacing
                        })
                    ).update(on: {
                        $0.alignment = .center
                    }),
...
extension TabBarItemTemplate.IconTitleBadge {
