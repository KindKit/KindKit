...
public extension Cache {
    ...
    func image(query: Query) -> Image? {
        guard let key = self._key(query) else { return nil }
        return self.image(key: key)
    }
    
    func image(query: Query, filter: Filter) -> Image? {
...
public extension Cache {
    ...
    func set(data: Data, image: Image, query: Query) throws {
        guard let key = self._key(query) else { return }
        try self.set(data: data, image: image, key: key)
    }
    
    func set(data: Data, image: Image, query: Query, filter: Filter) throws {
...
public extension Cache {
    ...
    func remove(query: Query) throws {
        guard let key = self._key(query) else { return }
        try self.remove(key: key)
    }
    
    func remove(query: Query, filter: Filter) throws {
        guard let key = self._key(query, filter) else { return }
        try self.remove(key: key)
    }
...
private extension Cache {
    ...
    func _setup() {
        AppState.default.onEnterBackground(target: self, regular: { $0.cleanupMemory() })
        AppState.default.onMemoryWarning(target: self, regular: { $0.cleanupMemory() })
    }
    
    func _destroy() {
        AppState.default.onEnterBackground(disconnect: self)
        AppState.default.onMemoryWarning(disconnect: self)
    }
    
    @inline(__always)
    func _key(_ query: Query) -> String? {
        let key = query.key
    ...
    @inline(__always)
    func _key(_ query: Query, _ filter: Filter) -> String? {
