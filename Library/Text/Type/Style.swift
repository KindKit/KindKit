...
extension Style : BatchUpdateTrait {
    
    public var isUpdateLocked: Bool {
        return self._updateCounter > 0
    }
...
public extension Style {
    ...
    func attribures(flags: TextFlags) -> [NSAttributedString.Key : Any] {
        ...
        if let fontColor = fontColor {
            attributes[.foregroundColor] = fontColor.handle
        }
        if let strokeColor = self.strokeColor {
            attributes[.strokeWidth] = self.strokeWidth ?? 1
            attributes[.strokeColor] = strokeColor.handle
        }
        if flags.contains(.underline) == true {
            ...
            if let underlineColor = underlineColor {
                ...
                attributes[.underlineColor] = underlineColor.handle
            }
        }
        if flags.contains(.strikethrough) == true {
            ...
            if let strikethroughColor = strikethroughColor {
                ...
                attributes[.strikethroughColor] = strikethroughColor.handle
            }
        }
