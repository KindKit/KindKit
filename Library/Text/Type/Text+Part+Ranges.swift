...
extension Text.Part {
    ...
    public struct Ranges< Value : Hashable & Equatable & Sendable > {
...
extension Text.Part.Ranges : Hashable {
}

extension Text.Part.Ranges : Equatable {
}

extension Text.Part.Ranges : Sendable {
}

public extension Text.Part.Ranges {
    ...
    mutating func clear(
        ...
        while index != self.items.endIndex {
            ...
            if range == item.range {
                ...
                do {
                    let range = Text.Part.Range(lower: range.upper, upper: item.range.upper)
    ...
    @inlinable
    func item(at index: Text.Part.Index) -> Item? {
    ...
    @inlinable
    func items(of range: Text.Part.Range) -> [Item] {
    ...
    @inlinable
    func range(of range: Text.Part.Range) -> Swift.Range< Int >? {
...
fileprivate extension Text.Part.Ranges {
    ...
    mutating func set(
        ...
        if range == item.range {
            ...
            if ld > 0 && td > 0 {
                ...
                do {
                    let range = Text.Part.Range(lower: item.range.lower, upper: range.lower)
