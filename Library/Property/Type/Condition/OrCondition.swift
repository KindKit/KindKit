...
public final class OrCondition< Left : Condition, Right : Condition > : OperatorCondition {
    ...
    public init(
        ...
        self.left.onChanged(target: self, regular: { target, _ in target.requestChange() })
        self.right.onChanged(target: self, regular: { target, _ in target.requestChange() })
    }
...
fileprivate extension OrCondition {
