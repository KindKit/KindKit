...
public final class FileTarget : Target {
    ...
    public init(
        ...
        self._queue = .init(label: "KindLog.FileTarget")
    ...
    public func log(message: Message) {
        self._queue.async(execute: { [weak self] in
            ...
            let line = "[\(marker)]: [\(message.category)]: \(message.string(options: .pretty))\r"
...
extension FileTarget : @unchecked Sendable {
}

fileprivate extension FileTarget {
