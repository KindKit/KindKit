...
extension Player {
    ...
    final class Observer : NSObject {
        ...
        func _change(time: CMTime) {
            ...
            handler.change(time: .init(cmTime: time))
        }
            ...
            if let item = object as? AVPlayerItem {
                ...
                case "playbackLikelyToKeepUp":
                    DispatchQueue.main.async(execute: {
                        ...
                        handler.change(time: .init(cmTime: item.currentTime()))
                    })
...
extension Player.Observer : @unchecked Sendable {
}
