...
extension DateInputView {
    ...
    struct Reusable : IReusable {
        ...
        typealias Owner = DateInputView
        typealias Content = KKDateInputView
...
final class KKDateInputView : UITextField {
    
    weak var kkDelegate: KKDateInputViewDelegate?
    let kkAccessoryView: KKAccessoryView
    var kkFormatter: KindString.DateFormatter? {
        didSet {
...
extension KKDateInputView {
    
    final class KKAccessoryView : UIInputView {
        
        weak var kkInput: KKDateInputView?
...
extension KKDateInputView {
    ...
    func update(view: DateInputView) {
    ...
    func update(mode: DateInputView.Mode) {
        self.kkPickerView.datePickerMode = mode.datePickerMode
    }
    
    func update(formatter: KindString.DateFormatter) {
        self.kkFormatter = formatter
    }
    
    func update(minimum: Date?) {
        self.kkPickerView.minimumDate = minimum
    }
    
    func update(maximum: Date?) {
        self.kkPickerView.maximumDate = maximum
    }
    
    func update(default: Date?) {
    ...
    func update(placeholderFont: Font?) {
        self.kkPlaceholderFont = placeholderFont?.native
    }
    
    func update(placeholderColor: Color?) {
    ...
    func update(toolbar: InputToolbarView?) {
...
private extension KKDateInputView {
    
    func _applyText() {
        if let value = self.kkValue {
            let formatter = self.kkFormatter ?? KindString.DateFormatter()
...
extension KKDateInputView : UITextFieldDelegate {
