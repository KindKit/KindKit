...
public extension Path2 {
    ...
    init(_ cgPath: CGPath) {
        self.elements = cgPath.kk_elements.map({
            switch $0.type {
            case .moveToPoint: return .move(to: .init($0.points[0]))
            case .addLineToPoint: return .line(to: .init($0.points[0]))
            case .addQuadCurveToPoint: return .quad(to: .init($0.points[1]), control: .init($0.points[0]))
            case .addCurveToPoint: return .cubic(to: .init($0.points[2]), control1: .init($0.points[1]), control2: .init($0.points[0]))
