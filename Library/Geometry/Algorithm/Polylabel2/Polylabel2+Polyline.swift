...
extension Polylabel2 {
    
    public static func find(polyline: Polyline2) -> Point {
        
        func polylabel(polygon: [[[Coordinate]]], precision: Coordinate = 1) -> [Coordinate] {
            var minX = Coordinate.leastNormalMagnitude
            var minY = Coordinate.leastNormalMagnitude
            var maxX = Coordinate.greatestFiniteMagnitude
            var maxY = Coordinate.greatestFiniteMagnitude
        ...
        func compareMax(a: Cell, b: Cell) -> Coordinate {
            return b.max - a.max
        }
        
        func getCentroidCell(polygon:[[[Coordinate]]]) -> Cell {
            var area: Coordinate = 0
            var x: Coordinate = 0
            var y: Coordinate = 0
        ...
        func toMultiArray(corners: [Point]) -> [[Coordinate]] {
            return corners.map { [$0.x, $0.y] }
        }
        
        let polygon: [[[Coordinate]]] = [toMultiArray(corners: polyline.corners)]
    ...
    fileprivate struct Queue {
        ...
        var data: [Cell]
        var compare: (Cell, Cell) -> Coordinate
        var length: Int {
            self.data.count
        }
        
        init(data: [Cell], compare: @escaping (Cell, Cell) -> Coordinate) {
            self.data = data
    ...
    fileprivate struct Cell {
        ...
        var max: Coordinate {
            self.d + self.h * Coordinate(2).sqrt
        }
        
        func pointToPolygonDist(x: Coordinate, y: Coordinate, polygon: [[[Coordinate]]]) -> Coordinate {
            var inside = false
            var minDistSq = Coordinate.greatestFiniteMagnitude
        ...
        func getSegDistSq(px: Coordinate, py: Coordinate, a: [Coordinate], b: [Coordinate]) -> Coordinate {
            var x = a[0]
            var y = a[1]
            var dx = b[0] - x
            var dy = b[1] - y
...
public extension Polyline2 {
    ...
    func visualCenter() -> Point  {
        return Polylabel2.find(polyline: self)
    }
