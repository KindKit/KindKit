...
struct Bezier {
    ...
    static func squaredLength(_ closure: (Coordinate) -> Coordinate) -> Coordinate {
        let tv = Self.Tvalues
        let cv = Self.Cvalues
        let count = tv.count
        let z = Coordinate(0.5)
        var s = Coordinate(0.0)
        for i in 0 ..< count {
            s += cv[i] * closure(z * tv[i] + z)
        }
        return .init(z * s)
    }
    
    static func crt(_ v: Coordinate) -> Coordinate {
        return (v < 0) ? -pow(-v, 1.0 / 3.0) : pow(v, 1.0 / 3.0)
    }
    
    static func droots(_ p0: Coordinate, _ p1: Coordinate, closure: (Coordinate) -> Void) {
        guard p0 != p1 else { return }
        closure(p0 / (p0 - p1))
    }
    
    static func droots(_ p0: Coordinate, _ p1: Coordinate, _ p2: Coordinate, closure: (Coordinate) -> Void) {
        let d = p0 - (2.0 * p1) + p2
    ...
    static func droots(_ p0: Coordinate, _ p1: Coordinate, _ p2: Coordinate, _ p3: Coordinate, closure: (Coordinate) -> Void) {
...
private extension Bezier {
    ...
    static let Tvalues: ContiguousArray< Coordinate > =  [
    ...
    static let Cvalues: ContiguousArray< Coordinate > =  [
