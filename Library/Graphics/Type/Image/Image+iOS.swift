...
public extension Image {
    ...
    init(
        ...
        if let renderingMode = renderingMode {
            self.handle = image.withRenderingMode(renderingMode)
        } else {
            self.handle = image
        }
    ...
    init(
        ...
        self.handle = UIImage(cgImage: cgImage)
        self.size = Size(self.handle.size)
        self.scale = Double(self.handle.scale)
    }
    
    @MainActor
    init?(size: Size, scale: Double? = nil, color: Color) {
        ...
        self.handle = UIImage(cgImage: image)
...
public extension Image {
    ...
    var grayscale: Image? {
        ...
        filter.setValue(CIImage(image: self.handle), forKey: kCIInputImageKey)
        if let output = filter.outputImage, let cgImage = context.createCGImage(output, from: output.extent) {
            return .init(UIImage(cgImage: cgImage, scale: self.handle.scale, orientation: self.handle.imageOrientation))
        }
        return nil
    }
...
public extension Image {
    ...
    func pngData() -> Data? {
        return self.handle.pngData()
    }
    
    func with(renderingMode: UIImage.RenderingMode) -> Image {
        return .init(self.handle.withRenderingMode(renderingMode))
    }
    ...
    func unrotate(maxResolution: Double) -> Image {
        guard let imgRef = self.handle.cgImage else {
            return self
        }
        ...
        let orientation = self.handle.imageOrientation
    ...
    func scaleTo(
        ...
        guard let cgImage = self.handle.cgImage else {
            return nil
        }
                ...
                if let frameProperties = CGImageSourceCopyPropertiesAtIndex(source, index, nil) as? [AnyHashable : Any] {
                    if let gifInfo = frameProperties[kCGImagePropertyGIFDictionary] as? [AnyHashable : Any] {
                        if let gifDelayTime = gifInfo[kCGImagePropertyGIFDelayTime] as? NSNumber {
                            frameDuration = Time(timeInterval: gifDelayTime.doubleValue)
                        }
                    }
                }
                frameDurations.append(frameDuration)
            }
            var totalDuration = Time.zero
