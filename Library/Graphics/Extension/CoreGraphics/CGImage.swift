...
public extension CGImage {
    ...
    func kk_compare(
        ...
        return self.kk_compare(
            expected: expected,
            tolerance: tolerance.value.cgFloat
        )
    }
    ...
    func kk_compare(
        ...
        guard let expectedContext = CGContext(data: expectedPixelsRaw, width: imageSize.width.int, height: imageSize.height.int, bitsPerComponent: bitsPerComponent, bytesPerRow: bytesPerRow, space: expectedColorSpace, bitmapInfo: bitmapInfo.rawValue) else {
            expectedPixels.deallocate()
            originPixels.deallocate()
            return false
        }
        guard let originContext = CGContext(data: originPixelsRaw, width: imageSize.width.int, height: imageSize.height.int, bitsPerComponent: bitsPerComponent, bytesPerRow: bytesPerRow, space: originColorSpace, bitmapInfo: bitmapInfo.rawValue) else {
            expectedPixels.deallocate()
            originPixels.deallocate()
            return false
        }
