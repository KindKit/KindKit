...
@Monadic
public final class CheckField : Field, MandatoryFieldTrait {
    ...
    @MonadicField
    public var policy: SequencePolicy = .one {
        didSet {
    ...
    @MonadicField
    public var fields: [any Field & SelectedFieldTrait] {
    ...
    @MonadicField
    public var selectedFields: [any Field & SelectedFieldTrait] {
        set { self.selectedFieldsProperty.value = newValue }
        get { self.selectedFieldsProperty.value }
    }
    
    public let selectedFieldsProperty: FieldsProperty
    ...
    public init(
        ...
        self.selectedFieldsProperty = .init(scope: scope)
        do {
        ...
        do {
            ...
            self._resultProperty = .init(
                scope: scope,
                dependencies: [ self.validProperty, self.selectedFieldsProperty ],
...
fileprivate extension CheckField {
    ...
    func _setup() {
        ...
        self.fieldsProperty.onChanged(
            ...
            regular: { target, change in
                ...
                for field in diff.removed {
                    ...
                    field.validProperty.onChanged(disconnect: target)
                    field.parent = nil
                    field.form = nil
                }
                for field in diff.added {
                    target._focusFieldsProperty.append(dependency: field.focusFieldsProperty)
                    field.validProperty.onChanged(target: target, regular: { [weak field] in
                        guard let field = field else { return }
                        $0._onSelect(field: field)
                    })
                    field.parent = target
        ...
        self.selectedFieldsProperty.onChanged(
            target: self,
            regular: { target, change in
                let diff = change.old.kk_difference(change.new, where: { $0 === $1 })
                for field in diff.removed {
        ...
        self._validCallback.callback = RegularTargetCallback(
            ...
            callback: { target in
                ...
                return target.policy.check(fields: target.selectedFields)
            }
        )
    ...
    func _onSelect(field: any Field & SelectedFieldTrait) {
        if field.isSelected == true {
            self.selectedFields.append(field)
            if let max = self.policy.max {
                while self.selectedFields.count > max {
                    self.selectedFields.removeFirst()
                }
            }
        } else if let index = self.selectedFields.firstIndex(where: { $0 === field }) {
            self.selectedFields.remove(at: index)
        }
    }
